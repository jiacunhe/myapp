...
BEGIN
	...
	REPEAT
		...
		IF NOT done THEN
			INSERT INTO t_user_apply_package(userId,assignorId,yearMonth,businessId,assignQuantity,usedQuantity,remark) VALUES(t_userId,t_assignorId,DATE_FORMAT(NOW(),'%Y%m'),t_businessId,t_quantity,0,NULL);
...
BEGIN
	...
	DECLARE t_status CHAR(1);
	DECLARE t_usedMonths INT;
	DECLARE cur_allot_immediate CURSOR FOR SELECT id,userId,assignorId,businessId,quantity,durationMonths,createTime,effectiveType,STATUS,usedMonths FROM t_user_assign_package WHERE STATUS ='1'  AND (usedMonths >0 OR effectiveType ='2');
	DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done = 1;
	OPEN cur_allot_immediate;
	REPEAT
		FETCH cur_allot_immediate INTO t_assign_id,t_userId,t_assignorId,t_businessId,t_quantity,t_durationMonths,t_createTime,t_effectiveType,t_status,t_usedMonths;
